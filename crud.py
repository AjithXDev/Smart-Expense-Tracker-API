...
def process_recurring_sync(payload: dict, conn):
    ...
    cursor.execute("SELECT id, title, category, amount, next_date, frequency, payment_method FROM recurring_expenses WHERE user_id=?", (user_id,))
    ...
    for r in recurm:
        ...
        while n_date <= today_str:
            cursor.execute(
                "INSERT INTO expenses(title, category, amount, date, payment_method, user_id) VALUES (?,?,?,?,?,?)",
...
def register(x:schemas.UserRegister,conn=Depends(database.get_db)):
    ...
    cursor.execute("SELECT * FROM users WHERE LOWER(username)=? OR LOWER(mail)=?", (username_lower, x.mail.lower()))
    ...
    cursor.execute(
        "INSERT INTO users(username,password,mail,name) values(?,?,?,?)",(x.username,hashed_password,x.mail,x.name)
    )
    conn.commit()
...
def login(x:schemas.UserLogin,conn=Depends(database.get_db)):
    ...
    cursor.execute(
        "SELECT * FROM users WHERE LOWER(username)=? OR LOWER(mail)=?", (login_id, login_id)
    )
...
def get_user_profile(payload:dict=Depends(auth.verify_token), conn=Depends(database.get_db)):
    cursor=conn.cursor()
    userid=payload["user_id"]
    cursor.execute("SELECT id, username, mail, name FROM users WHERE id=?",(userid,))
    user = cursor.fetchone()
    if not user:
...
def add_recurring_expense(x: schemas.RecurringExpenseAdd, payload: dict=Depends(auth.verify_token), conn=Depends(database.get_db)):
    ...
    user_id = payload["user_id"]
    cursor.execute(
        "INSERT INTO recurring_expenses(title, category, amount, next_date, frequency, payment_method, user_id) VALUES (?,?,?,?,?,?,?)",
...
def get_recurring_expenses(payload: dict=Depends(auth.verify_token), conn=Depends(database.get_db)):
    cursor=conn.cursor()
    user_id = payload["user_id"]
    cursor.execute("SELECT id, title, category, amount, next_date, frequency, payment_method FROM recurring_expenses WHERE user_id=? ORDER BY id DESC", (user_id,))
    rows = cursor.fetchall()
    res = []
    for r in rows:
        res.append({
...
def delete_recurring_expense(id: int, payload: dict=Depends(auth.verify_token), conn=Depends(database.get_db)):
    cursor=conn.cursor()
    user_id = payload["user_id"]
    cursor.execute("DELETE FROM recurring_expenses WHERE id=? AND user_id=?", (id, user_id))
    if cursor.rowcount == 0:
